...
class NavigationModel {
  ...
  factory NavigationModel(jsonStr) => jsonStr == null ? null : jsonStr is String ? new NavigationModel.fromJson(json.decode(jsonStr)) : new NavigationModel.fromJson(jsonStr);
  ...
  NavigationModel.fromJson(jsonRes) {
    ...
    for (var dataItem in data == null ? [] : jsonRes['data']){
      data.add(dataItem == null ? null : new NavigationData.fromJson(dataItem));
    }
  }
...
class NavigationData {
  ...
  int cid;
  String name;
  List<NavigationArticle> articles;

  NavigationData.fromParams({this.cid, this.name, this.articles});

  NavigationData.fromJson(jsonRes) {
    cid = jsonRes['cid'];
    name = jsonRes['name'];
    articles = jsonRes['articles'] == null ? null : [];

    for (var articlesItem in articles == null ? [] : jsonRes['articles']){
      articles.add(articlesItem == null ? null : new NavigationArticle.fromJson(articlesItem));
    }
  }
...
class NavigationArticle {
  ...
  NavigationArticle.fromParams({this.chapterId, this.courseId, this.id, this.publishTime, this.superChapterId, this.type, this.userId, this.visible, this.zan, this.collect, this.fresh, this.apkLink, this.author, this.chapterName, this.desc, this.envelopePic, this.link, this.niceDate, this.origin, this.projectLink, this.superChapterName, this.title, this.tags});

  NavigationArticle.fromJson(jsonRes) {
